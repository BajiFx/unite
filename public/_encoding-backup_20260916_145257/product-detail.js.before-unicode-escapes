...
// ============================================================
//  ORDER RELATED PRODUCTS â€” Option B
// ============================================================
...
function renderDetail(product, related) {
  ...
  const stockHtml = `
    <div class="stock-info">
      ${stockDisplay > 0 ?
        `<span class="in-stock">âœ“ In Stock (${stockDisplay} available)</span>` :
  ...
  if (ratingValue > 0) {
    ratingHtml = `
      <div class="rating-section">
        <span class="stars">${'â­'.repeat(Math.min(fullStars, 5))}</span>
  ...
  const categoryChipHtml = product.product_category_name
    ? `<div class="product-category-chip" title="${product.product_category_name}">${product.product_category_icon || 'ðŸ“¦'} ${product.product_category_name}</div>`
    : '';
  ...
  if (product.isFlashSale) badgesHtml += `<span class="badge tag-flash">ðŸ”¥ Flash Sale</span>`;
  if (product.isNewArrival) badgesHtml += `<span class="badge tag-new">ðŸ†• New</span>`;
  ...
  if (returnEnabled) {
    ...
    returnPolicyHtml = `
      <div class="return-policy" style="background:linear-gradient(145deg, #fef2f2, #fee2e2); border-left-color:#ef4444; color:#991b1b;">
        <strong>âŒ Non-Returnable:</strong> This item is final sale and cannot be returned.
      </div>
    `;
  }
  ...
  if (media.length > 0 && media[currentMediaIndex]) {
    ...
    mainMediaHtml = '<div class="no-image">ðŸ“¦</div>';
  }
  ...
  if (related && related.length > 0) {
    relatedHtml = related.map(p => `
      <div class="related-item"
        ...
        ${p.image ? `<img src="${p.image}" alt="${p.name}">` : `<div class="no-image">ðŸ“¦</div>`}
      ...
      <div class="detail-info">
        ...
        <div class="qty-section">
          ...
          <div class="qty-control">
            <button onclick="changeDetailQty(-1)">âˆ’</button>
        ...
        <div class="button-group">
          <button class="btn-add-large ${btnClass}" onclick="addVariantToCart()">ðŸ›’ ${btnText}</button>
...
// ============================================================
//  COLOUR GALLERY â€” interaction wiring
// ============================================================
...
function selectColour(key, options) {
  ...
  if (stockInfo) {
    ...
    stockInfo.innerHTML = stock > 0
      ? `<span class="in-stock">âœ“ In Stock (${stock} available)</span>`
      : `<span class="out-of-stock">âœ• Out of Stock</span>`;
  }
  ...
  if (btn) {
    ...
    btn.textContent = isInCart ? 'ðŸ›’ Add More' : 'ðŸ›’ Add to Cart';
...
function addVariantToCart() {
  ...
  saveCart(cart);
  updateCartBadge();
  showToast(`âœ… Added ${detailQty} "${currentProduct.name}" (${variantName}) to cart!`, 'success');
  detailQty = 1;
  const span = document.getElementById('detailQty');
    ...
    footer.innerHTML = `
      <div class="bidhaa-legal-footer-inner">
        ...
        <div class="bidhaa-legal-footer-thanks" id="footerThankYou">
          <span class="bidhaa-legal-footer-thanks-icon" aria-hidden="true">ðŸ™</span>
