...
//
//  Required: name, phone, password, confirm
//  Optional: email (kept for password recovery; the customer
//            can add it later from Profile)
//
//  Username is auto-generated by the server from the name.
//  The server returns it in the response so we can show it once,
//  and the account page shows it again on the Profile section.
// ============================================================
...
async function handleCustomerRegister() {
  ...
  // --- Required field check (name, phone, password, confirm) ---
  ...
  // --- Optional email check (only if the customer typed one) ---
  if (email) {
  ...
  try {
    ...
    if (res.ok && data.success) {
      ...
      // Clear the form so a subsequent registration does not
      // carry stale values.
      ...
      // Tell the customer their auto-generated username so they
      // know what to use if they ever log in by username. The
      // account page shows the same value in the Profile section.
...
console.log('✅ Index.js loaded successfully (Section 6 customer registration simplified; strict round-robin, 4s images / 20s videos, video with sound)');